section .data
    msg db "Hello, World! This is exciting!", 0Ah, 0

section .text
global _start

_start:
    mov rdi, msg ; rdi will be 1 anyways when calling sys_write (code 1), so lets use rdi as
                 ; a temporary storage for the string's memory address 
    mov rax, rdi ; same with rax, but in this instance we will also do some editing on rax

increment: ; this indicator (and the ones after this) define a procedure, basically a function 
